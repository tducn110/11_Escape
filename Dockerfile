...
# The exact version pinned in package.json is installed directly: corepack's
# loader cannot execute pnpm 11 binaries on Node 22.22
# (ERR_VM_DYNAMIC_IMPORT_CALLBACK_MISSING), and the frozen lockfile keeps the
# dependency graph byte-identical to the local toolchain.
RUN npm install --global --no-fund --no-audit pnpm@11.9.0

# Dependencies ship their platform binaries as optionalDependencies
# (esbuild, @tailwindcss/oxide), so no lifecycle script is ever required.
# Skipping them also keeps installs hermetic and fast.
...
RUN pnpm typecheck \
    ...
    && pnpm levels:verify \
    && pnpm assets:verify \
    && pnpm verify:wink-bridge \
    && pnpm build \
    && test -d "/app/${BUILD_OUTPUT_DIR}"
